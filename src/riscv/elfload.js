...
function loadElf(binfile, filename, filesList) {
  ...
  var elf = {};
  var magic = ((binfile.charCodeAt(0) & 0xFF) << 24) |
      ((binfile.charCodeAt(1) & 0xFF) << 16) |
  ...
  for (var i = 0; i < section_headers.length; i++) {
    ...
    if ((((section_headers[i]['flags'].getLowBits() >> 1) & 0x1) == 0x1) &&
        (section_headers[i]['type'] != 8)) {
      for (var j = 0; j < section_headers[i]["size"].getLowBits(); j++) {
        RISCV.memory[((section_headers[i]['addr'].getLowBits() & 0x7FFFFFFF) +
            j) >> 2] |= (binfile.charCodeAt(
            (section_headers[i]['offs'].getLowBits() | 0) + j) & 0xFF) <<
            ((j & 0x3) << 3);
      }
    } else if ((((section_headers[i]['flags'].getLowBits() >> 1) & 0x1) ==
        0x1) && (section_headers[i]['type'] == 8)) {
...
function bytes_to_long(input, addr, numbytes, end) {
  if (end == 'b') {
    return new Long(bytes_to_int(input, addr + 4, 4, end),
        bytes_to_int(input, addr, 4, end));
  } else if (end == 'l') {
    return new Long(bytes_to_int(input, addr, 4, end),
        bytes_to_int(input, addr + 4, 4, end));
  } else {
