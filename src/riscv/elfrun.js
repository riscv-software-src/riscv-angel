  ...
  for (var i = 1; ; i++) {
    ...
    if (RISCV.pc == (0x80152b58 | 0)) {
      if (readTest.length) {
        ...
        RISCV.priv_reg[PCR['CSR_FROMHOST']['num']] = new Long(
            0x100 | (readTest.shift().charCodeAt(0) & 0xFF), 0x01000000);
        RISCV.priv_reg[PCR['CSR_STATUS']['num']] = RISCV.priv_reg[PCR['CSR_STATUS']['num']] |
            0x40000000;
        ...
        if (tryCount == stopCount) {
          ...
          if (lastCharWritten == 0x1) {
            lastCharWritten = 0x0;
            RISCV.priv_reg[PCR['CSR_COMPARE']['num']] = RISCV.priv_reg[PCR['CSR_COUNT']['num']] +
                100000;
            stopCount = 200000;
          } else {
            stopCount = 10000;
